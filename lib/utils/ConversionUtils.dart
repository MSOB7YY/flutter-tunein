...
class ConversionUtils {
  ...
  static Future<List<int>> FileUriTo8Bit(String uri,
      {File? fileInstead}) async {
    assert(uri != null || fileInstead != null,
        "one of uri and file needs to be supplied");
    if (fileInstead != null) {
  ...
  static Future<Uint8List> fromWidgetGlobalKeyToImageByteList(
      GlobalKey widgetlobalKey) async {
    assert(widgetlobalKey != null, "You can't pass a null global key");
    RenderRepaintBoundary? boundary = widgetlobalKey.currentContext!
  ...
  static double songListToDuration(List<Tune> songs) {
    ...
    songs.forEach((elem) {
      finalDuration += elem.duration ?? 0;
    });

    return finalDuration.toDouble();
  }
