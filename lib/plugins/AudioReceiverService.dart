...
class AudioReceiverService {
  ...
  Map<String, StreamSubscription> _audioPositionSub =
      new Map<String, StreamSubscription>();
  Map<String, StreamSubscription> _audioStateChangeSub =
      new Map<String, StreamSubscription>();
  Map<String, StreamSubscription> _audioPlaybkacKeysSub =
      new Map<String, StreamSubscription>();
  ...
  Future playSong(String uri,
    ...
    return _audioPlayer.play(uri,
        title: title, album: album, albumArt: albumArt, author: artist);
  }
  ...
  closeAllSubs() {
    if (_audioPlaybkacKeysSub != null)
      _audioPlaybkacKeysSub.forEach((key, element) {
        element.cancel();
        _audioPlaybkacKeysSub.remove(key);
      });
    if (_audioPositionSub != null)
      _audioPositionSub.forEach((key, element) {
        element.cancel();
        _audioPositionSub.remove(key);
      });
    if (_audioStateChangeSub != null)
      _audioStateChangeSub.forEach((key, element) {
        element.cancel();
        _audioStateChangeSub.remove(key);
      });
  }
  ...
  useNotification({bool? useNotification, bool? cancelWhenPlayingStops}) {
    return _audioPlayer.useNotificationMediaControls(
        useNotification, cancelWhenPlayingStops);
  }
