...
class notificationControlService {
  ...
  Future show({
    ...
    String messageToSend = json.encode({
      ...
      "bgImageBackgroundColor": bgImageBackgroundColor != null
          ? bgImageBackgroundColor.value.toString()
          : Colors.white,
      "play": play ?? false,
      "image": image ?? null,
      "BitmapImage": BitmapImage ?? null,
      "titleColor":
          titleColor != null ? titleColor.value.toString() : Colors.white,
      "subtitleColor":
          subtitleColor != null ? subtitleColor.value.toString() : Colors.white,
      "iconColor":
          iconColor != null ? iconColor.value.toString() : Colors.white,
      "bigLayoutIconColor": bigLayoutIconColor?.value.toString(),
      "bgColor": bgColor != null ? bgColor.value.toString() : Colors.black
    });
    sendNewIsolateCommand(command: "showNotification", message: messageToSend)
        .then((value) {
      return value;
    });
  }
  ...
  Future hide() async {
    return await sendNewIsolateCommand(
        command: "hideNotification", message: "");
  }
  ...
  Future setNotificationTo(bool value) async {
    return await sendNewIsolateCommand(
        command: "setTo", message: value.toString());
  }

  Future setNotificationStatusIcon(String iconName) async {
    return await sendNewIsolateCommand(
        command: "setStatusIcon", message: iconName.toString());
  }

  Future setNotificationTitle(String iconName) async {
    return await sendNewIsolateCommand(
        command: "setTitle", message: iconName.toString());
  }

  Future setNotificationSubtitle(String iconName) async {
    return await sendNewIsolateCommand(
        command: "setSubtitle", message: iconName.toString());
  }

  Future toggleNotificationPlayPause() async {
    return await sendNewIsolateCommand(
        command: "togglePlaypauseButton", message: "");
  }
}
