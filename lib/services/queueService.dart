...
enum QueueState { STARTING, PAUSED, ONGOING, STOPPED }

class QueueService {
  late BehaviorSubject<List<QueueItem>> _queue$;

  BehaviorSubject<List<QueueItem>> get queue$ => _queue$;

  late int currentIndexProcess = 0;
  late QueueState currentQueueState = QueueState.STOPPED;
  late Duration queueInterval = Duration(minutes: 1);
  Timer? currentRunner;
  late List<QueueItem> AlreadyExecutedItems = [];
  late VoidCallback onQueueEndCallback;
  ...
  Future<bool> startQueue() async {
    if (currentQueueState != QueueState.ONGOING &&
  ...
  Future<bool> pauseQueue() async {
    if (currentQueueState != QueueState.PAUSED &&
  ...
  QueueRunnerStart() {
    ...
    currentRunner = Timer.periodic(queueInterval, (timer) {
      ...
      if (queue[currentIndexProcess].execute != null) {
        print(
            "will execute processs of item ${queue[currentIndexProcess].name}");
        queue[currentIndexProcess].execute().then((data) {
          AlreadyExecutedItems.add(queue[currentIndexProcess]);
          if (data == true) {
            print(
                " execute processs of item ${queue[currentIndexProcess].name} has been done");
...
enum QueueItemState { IDLE, STARTED, ERRORED, COMPLETED }
...
class QueueItem {
  ...
  QueueItem(
      ...
      this.state = QueueItemState.IDLE}) {
    this.id = Uuid().v1();
  }
}
