...
enum DeviceSearchingState { SEARCHING, NOT_SEARCHING }

class CastService {
  late BehaviorSubject<CastState> _castingState;
  late BehaviorSubject<DeviceSearchingState> _deviceSearchingState;
  late BehaviorSubject<PlayerState> _castingPlayerState;
        ...
        if (_currentDeviceToBeUsed.value != null) {
          positionTimer = Timer.periodic(Duration(seconds: 1), (Timer) async {
            if (castingState.value == CastState.CASTING &&
                castingPlayerState.value == PlayerState.playing) {
              UpnPPlugin.getPositionInfo(
                      service:
                          await UpnPPlugin.getAVTTransportServiceFromDevice(
                              _currentDeviceToBeUsed.value))
                  .then((positionData) {
                String position = positionData["RelTime"];
                Duration positionInDuration =
                    FormatPositionToDuration(position);
                if ((_currentPosition.value == null ||
                    positionInDuration.inSeconds !=
      ...
      if (feedState) {
        if (castingPlayerStateTimer == null) {
          if (_currentDeviceToBeUsed.value != null) {
            ...
            PlayerStateStream = await UpnPPlugin.subscribeToService(
                service: await UpnPPlugin.getAVTTransportServiceFromDevice(
                    _currentDeviceToBeUsed.value),
                newSub: true);
  ...
  Future castAndPlay(Tune songToCast,
      {Tune? nextSong, bool SingleCast = false, Device? deviceToUse}) async {
    if (_castingState.value == CastState.CASTING || SingleCast) {
      await registerSongForServing(songToCast);
      if (songToCast.albumArt != null) {
        await registerArtForService(
            "art${songToCast.id}", songToCast.albumArt!);
      }

      String currentIP = await platformService.getCurrentIP();
      String currentPort = SettingService.getCurrentMemorySetting(
          SettingsIds.SET_OUT_GOING_HTTP_SERVER_PORT);
      String newURI =
          "http://${currentIP}:${currentPort}/file?fileID=${songToCast.id}.mp3";
      print(newURI);
      String newArtURI =
          "http://${currentIP}:${currentPort}/file?fileID=art${songToCast.id}.jpg";
      CastItem newItemToCast = CastItem(
          uri: newURI,
          name: songToCast.title.toString(),
          coverArtUri: songToCast.albumArt != null ? newArtURI : null,
          id: songToCast.id);

      Device currentDev = deviceToUse ?? _currentDeviceToBeUsed.value;
      UpnPPlugin.setCurrentURI(
        service: await currentDev
        ...
        Duration: DurationToFormatPosition(
            Duration(milliseconds: songToCast.duration!)),
  ...
  Future stopCasting() async {
    await UpnPPlugin.stopCurrentMedia(
        service: await UpnPPlugin.getAVTTransportServiceFromDevice(
            _currentDeviceToBeUsed.value));
  ...
  Future pauseCasting() async {
    await UpnPPlugin.pauseCurrentMedia(
        service: await UpnPPlugin.getAVTTransportServiceFromDevice(
            _currentDeviceToBeUsed.value));
    return;
  }
  ...
  Future play() async {
    await UpnPPlugin.playCurrentMedia(
        service: await UpnPPlugin.getAVTTransportServiceFromDevice(
            _currentDeviceToBeUsed.value));
    return;
  }
  ...
  Future seek(Duration durationToSeekTo) async {
    await UpnPPlugin.seekPostion(
        service: await UpnPPlugin.getAVTTransportServiceFromDevice(
            _currentDeviceToBeUsed.value),
        position: DurationToFormatPosition(durationToSeekTo));
    return;
  }
  ...
  FormatPositionToDuration(String position) {
    ...
    int seconds = (int.parse(splitChronos[0]) * (60 * 60)) +
        (int.parse(splitChronos[1]) * 60) +
        (int.parse(splitChronos[2]));
  ...
  _initStreams() {
    ...
    _currentPosition =
        BehaviorSubject<Duration>.seeded(Duration(milliseconds: 0));
    _castingPlayerState =
        BehaviorSubject<PlayerState>.seeded(PlayerState.stopped);
    _currentDeviceToBeUsed = BehaviorSubject<Device>.seeded(null);
    _deviceSearchingState = BehaviorSubject<DeviceSearchingState>.seeded(
        DeviceSearchingState.NOT_SEARCHING);
  }
