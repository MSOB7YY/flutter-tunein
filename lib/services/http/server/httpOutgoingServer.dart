...
class HttpOutgoingServer {
  ...
  Map<String, SimpleRequest>? callbacks = Map();

  HttpOutgoingServer(
      {bool doCreateServer = true,
      String? ip,
      String? port,
      bool newSharedServer = true,
      bool initiateListeningImmediately = true}) {
    if (doCreateServer) {
      createServer(ip ?? "0.0.0.0", port ?? "8090",
          shared: newSharedServer,
          initiateImmediately: initiateListeningImmediately);
    }
  }

  Future createServer(String ip, String port,
      {bool shared = true, bool initiateImmediately = true}) async {
    int intPort = int.parse(port,
        radix: 10, onError: (err) => throw "Server port is incorrect");
    try {
  ...
  Future<dynamic> stopCurrentHTTPServer({bool forceStop = false}) {
