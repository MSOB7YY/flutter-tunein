...
class fileService {
  ...
  saveFile(File? fileToBeSaved, String? data,
      {bool? writeBytes, List<int>? bytesToWrite, nativeSave = false}) async {
    if (fileToBeSaved != null) {
      if (fileToBeSaved.existsSync()) {
        if (writeBytes!) {
          if (nativeSave) {
            print("FILE IS IN SDCARD ? : ${isFileOnSDCard(fileToBeSaved)}");
            if (isFileOnSDCard(fileToBeSaved) != null) {
              await getSDCardAndPermissions();
            }
            return saveFileToPathNatively(
                fileToBeSaved.uri.toString(), bytesToWrite!);
          } else {
  ...
  FutureOr<bool?>? isFileOnSDCard(File? file) {
    ...
    String? sdCardName =
        memoryCachingService.getCacheItem(CachedItems.SDCARD_NAME);
  ...
  Future<dynamic> saveFileToPathNatively(String path, List<int> bytes) {
    ...
    return platform.invokeMethod("saveFileFromBytes",
        {"filepath": path, "bytes": Uint8List.fromList(bytes)});
  ...
  Future<bool> writeTags(Tune Song, {List<String>? comments}) async {
    final AttachedPicture pic = AttachedPicture(
        ...
        '${Song.album}.jpg',
        Song.albumArt != null
            ? await ConversionUtils.FileUriTo8Bit(Song.albumArt!,
                fileInstead: null)
            : []);
    List<Comment>? comms;
    if (comments != null) {
    ...
    final blocks =
        writer.write(await ConversionUtils.FileUriTo8Bit(Song.uri!), tag);

    await saveFile(File(Song.uri!), null,
        writeBytes: true, bytesToWrite: await blocks, nativeSave: true);
