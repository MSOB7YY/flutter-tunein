...
class MusicMetricsService {
  late BehaviorSubject<Map<MetricIds, dynamic>> _metrics;

  BehaviorSubject<Map<MetricIds, dynamic>> get metrics => _metrics;

  List<MapEntry<MetricIds, BehaviorSubject<dynamic>>>
  ...
  MapEntry<MetricIds, BehaviorSubject<dynamic>> addSinglSettingStream(
      MetricIds setting) {
    MapEntry<MetricIds, BehaviorSubject<dynamic>> newStream =
        MapEntry(setting, new BehaviorSubject<dynamic>.seeded(null));
  ...
  BehaviorSubject<dynamic> getOrCreateSingleSettingStream(MetricIds setting) {
    MapEntry<MetricIds, BehaviorSubject<dynamic>> existingStream =
        activeSingleSettingListeners
  ...
  Stream<Map<MetricIds, dynamic>> createSettingStreamOfASettingId(
      MetricIds setting) {
  ...
  incrementPlaylistPlaytimeOnSinglePlaylist(
      Playlist playlist, Duration durationToAdd) async {
    if (durationToAdd != null) {
      return;
    }
    Map<String, dynamic> PlayedTimeOnAllPlaylists =
        getCurrentMemoryMetric(MetricIds.MET_GLOBAL_PLAYLIST_PLAY_TIME);
    if (PlayedTimeOnAllPlaylists[playlist.id] != null) {
      ...
      updateSingleMetric(
          MetricIds.MET_GLOBAL_PLAYLIST_PLAY_TIME, PlayedTimeOnAllPlaylists);
    } else {
      PlayedTimeOnAllPlaylists[playlist.id!] =
          durationToAdd.inSeconds.toString();
      updateSingleMetric(
          MetricIds.MET_GLOBAL_PLAYLIST_PLAY_TIME, PlayedTimeOnAllPlaylists);
    }
  }
  ...
  Future<dynamic> addSongToLatestPlayedSongs(Tune song) {
    List<Tune> existingList =
        getCurrentMemoryMetric(MetricIds.MET_GLOBAL_LAST_PLAYED_SONGS);
    if (existingList.length == 10) {
      existingList.removeLast();
    }
    existingList.add(song);
    return updateSingleMetric(
        MetricIds.MET_GLOBAL_LAST_PLAYED_SONGS, existingList);
  }

  FutureOr<bool> removeSongFromLatestPlayedSongs(Tune? song,
    ...
    Tune? foundSong = existingList.firstWhere((element) {
      song!.title ?? title == element!.title || element.id == id;
      return true;
    }, orElse: () => null);
    ...
    return updateSingleMetric(
            MetricIds.MET_GLOBAL_LAST_PLAYED_SONGS, existingList)
        .then((value) => songRemoved);
  }

  void incrementPlayTimeOnSingleSong(
      Tune? song, Duration? durationToAdd) async {
    if (song != null && durationToAdd != null) {
      String currentGlobalTimeValue =
          getCurrentMemoryMetric(MetricIds.MET_GLOBAL_PLAY_TIME).toString();
      int numericValueOfGlobalPlayTime = int.parse(currentGlobalTimeValue);
      numericValueOfGlobalPlayTime += durationToAdd.inSeconds;
      updateSingleMetric(
          MetricIds.MET_GLOBAL_PLAY_TIME, numericValueOfGlobalPlayTime);

      Map<String, dynamic> PlayedTimeOnAllSongs =
      ...
      if (PlayedTimeOnAllSongs[song.id] != null) {
        ...
        updateSingleMetric(
            MetricIds.MET_GLOBAL_SONG_PLAY_TIME, PlayedTimeOnAllSongs);
      } else {
  ...
  void incrementPlayTimeOnSingleArtist(
      Artist artist, Duration durationToAdd) async {
    if (artist != null && durationToAdd != null) {
      Map<String, dynamic> PlayedTimeOnAllArtists =
          getCurrentMemoryMetric(MetricIds.MET_GLOBAL_ARTIST_PLAY_TIME);
      if (PlayedTimeOnAllArtists[artist.id] != null) {
        int numericValueOfSong = int.parse(PlayedTimeOnAllArtists[artist.id]);
        numericValueOfSong += durationToAdd.inSeconds;
        PlayedTimeOnAllArtists[artist.id.toString()] =
            numericValueOfSong.toString();
        updateSingleMetric(
            MetricIds.MET_GLOBAL_ARTIST_PLAY_TIME, PlayedTimeOnAllArtists);
      } else {
        PlayedTimeOnAllArtists[artist.id.toString()] =
            durationToAdd.inSeconds.toString();
        updateSingleMetric(
  ...
  void deleteAllMetricsOfSongs(List<Tune> songs) {
    Map<String, dynamic> globalSongTime =
        getCurrentMemoryMetric(MetricIds.MET_GLOBAL_SONG_PLAY_TIME);
    ...
    songs.forEach((element) {
      ...
      lasTPlayedSongs
          .removeWhere((lastPlayedSong) => lastPlayedSong.id == element.id);
    });
  ...
  void deleteAllMetricsOfArtist(List<Artist> artists) {
    Map<String, dynamic> globalArtistTime =
        getCurrentMemoryMetric(MetricIds.MET_GLOBAL_ARTIST_PLAY_TIME);
      ...
      MetricIds.values.toList().forEach((setting) {
        ...
        switch (getMetricStorageTye(setting)) {
          ...
          case List:
            {
              storedSettingValue =
                  _prefs.getStringList(getEnumValue(setting).toString());
              break;
            }
        }
  ...
  fetchSingleMetric(MetricIds metricId) async {
    ...
    try {
      ...
      metricsMap[metricId] = convertFromStorage(
          metricId, _prefs.getString(getEnumValue(metricId).toString()));
      _metrics.add(metricsMap);
    } catch (e) {
  ...
  Future updateSingleMetric(MetricIds metricId, dynamic value) async {
    ...
    try {
      ...
      if (valueToAdd is List) {
        await _prefs.setStringList(getEnumValue(metricId).toString(),
            convertToStorage(metricId, value));
      } else {
        await _prefs.setString(getEnumValue(metricId).toString(),
            convertToStorage(metricId, value));
      }
