...
import 'package:flutter_tunein_attempt3/models/playback.dart';
import 'package:flutter_tunein_attempt3/models/playerstate.dart';
import 'package:flutter_tunein_attempt3/plugins/AudioPluginService.dart';
...
import 'package:flutter_tunein_attempt3/services/dialogService.dart';
...
import 'package:flutter_tunein_attempt3/services/isolates/standardIsolateFunctions.dart';
...
class MusicService {
  ...
  late BehaviorSubject<MapEntry<PlayerState, Playlist?>>
      _currentPlayingPlaylist$;

  late BehaviorSubject<List<Album>> _albums$;
  late BehaviorSubject<List<Artist>> _artists$;
  late BehaviorSubject<Map<String, Artist>> _artistsImages$;
  late BehaviorSubject<MapEntry<PlayerState, Tune>> _playerState$;
  ...
  late AudioPluginService _audioPlayer;
  late notificationControlService _notificationService;
  late Nano _nano;
  late Tune? _defaultSong;
  late Map<String, Tune> SongList;
  late Map<int, Artist> ArtistList;
  late Map<int, Album> AlbumList;
  ...
  BehaviorSubject<MapEntry<PlayerState, Playlist?>>
      get currentPlayingPlaylist$ => _currentPlayingPlaylist$;
  ...
  showUI() async {
    ...
    SettingsService.getOrCreateSingleSettingStream(
            SettingsIds.SET_ANDROID_NOTIFICATION_PLAYBACK_CONTROL)
        .listen((value) {
      if (value == "true") {
        _audioPlayer.useNotification(
            useNotification: true, cancelWhenNotPlaying: false);
        _audioPlayer.showNotification();
      } else {
        _audioPlayer.useNotification(
            useNotification: false, cancelWhenNotPlaying: false);
    ...
    Rx.combineLatest2(
            ...
            SettingsService.getOrCreateSingleSettingStream(
                SettingsIds.SET_CUSTOM_NOTIFICATION_PLAYBACK_CONTROL),
            (a, b) => MapEntry<MapEntry<PlayerState, Tune>, String>(a, b))
        .listen((data) async {
      List<int> SongColors = await themeService.getThemeColors(data.key.value);
      Artist? artist = artistsImages$.value != null
          ? artistsImages$.value[data.key.value.artist]
          : null;
      if (data.value == "true") {
        switch (data.key.key) {
          ...
          case PlayerState.playing:
            _notificationService.show(
                ...
                bigLayoutIconColor:
                    artist!.colors != null && artist.colors.length != 0
                        ? Color(artist.colors[1]!)
                        : null,
                bgImage: data.key.value.artist != null && artist != null
                    ? artist.coverArt
                    : null,
                bgBitmapImage: artist.coverArt == null && artist != null
                    ? defaultBgImageBytes
                    : null,
                bgImageBackgroundColor: (artist != null &&
                        artist.colors != null &&
                        artist.colors.length != 0)
        ...
        await _notificationService.show(
            ...
            subtitleColor: Color(SongColors[1]).withAlpha(50),
            iconColor: Color(SongColors[1]),
            bigLayoutIconColor: artist != null &&
                    artist.colors != null &&
                    artist.colors.length != 0
                ? Color(artist.colors[1]!)
                : null,
            bgImage: artist != null && data.key.value.artist != null
                ? artist.coverArt
                : null,
            bgBitmapImage:
                artist != null && artist.coverArt == null && artist != null
                    ? defaultBgImageBytes
                    : null,
            bgImageBackgroundColor: (artist != null &&
                    artist.colors != null &&
  ...
  Future<bool> updateAlbums({List<String>? albumNames}) async {
    Map<String, Album> albums = AlbumList.map((key, value) =>
        MapEntry<String, Album>('${value.title}${value.artist}', value));
    int currentIndex = 0;
      ...
      if (albumNames!.contains(tune.album)) {
        if (albums["${tune.album}${tune.artist}"] != null) {
          albums["${tune.album}${tune.artist}"]!
              .songs
              .removeWhere((element) => element.id == tune.id);
          albums["${tune.album}${tune.artist}"]!.songs.add(tune);
        } else {
          albums["${tune.album}${tune.artist}"] =
              Album(currentIndex, tune.album!, tune.artist!, tune.albumArt!);
          albums["${tune.album}${tune.artist}"]!.songs.add(tune);
          currentIndex++;
        }
      }
    ...
    newAlbumList.forEach((album) {
      album.songs.sort((a, b) {
        ...
        if (a.numberInAlbum! < b.numberInAlbum!)
          return -1;
        else
          return 1;
      });
    ...
    ItemsList.forEach((Album album) {
      if (artists["${album.artist}"] != null) {
        ...
        artists["${album.artist}"] =
            new Artist(currentIndex, album.artist, "", "");
        artists["${album.artist}"]!.albums.add(album);
        currentIndex++;
      }
    });
  ...
  Future<List> updateArtist({List<String>? artistNames}) async {
    Map<String, Artist> artists = artists$.value
        .asMap()
        .map((key, value) => MapEntry<String, Artist>(value.name!, value));
    int currentIndex = 0;
    List<Album> ItemsList = _albums$.value;
  ...
  Future setAndroidNativeNotificationItem(
      {String? uri,
      String? title,
      String? album,
      String? artist,
      String? albumArt}) {
    return _audioPlayer.setItem(
        uri: uri!,
        artist: artist!,
        album: album!,
        albumArt: albumArt!,
  ...
  Future<int> rescanLibrary(context) async {
    ...
    if (!eq(newURIList, songs$.value.map((e) => e.uri).toList())) {
      List<String?> differenceSet =
          newURIList.toSet().difference(oldUriList.toSet()).toList();
      List<String?> DeleteSongsBasedOnDifference =
          oldUriList.toSet().difference(newURIList.toSet()).toList();
      List<Tune> differenceSongs =
          data.where((song) => differenceSet.contains(song.uri)).toList();

      Map<String, Artist> newOldArtistMap =
          oldArtists.asMap().map((key, value) {
      ...
      List<Tune> songsToDelete = oldSongsList
          .where((song) => DeleteSongsBasedOnDifference.contains(song.uri))
          .toList();
      if (DeleteSongsBasedOnDifference.length != 0) {
        songsToDelete.forEach((element) {
          ...
          newOldAlbumMap[element.album]!
              .songs
              .removeWhere((albumSongs) => albumSongs.uri == element.uri);
          newOldArtistMap[element.artist]!
              .albums
              .firstWhere((artistAlbum) => artistAlbum.title == element.album,
          ...
          if (newOldAlbumMap[element.album]!.songs.length == 0) {
            ...
            newOldArtistMap[element.artist]!.albums.removeWhere(
                (artistAlbum) => artistAlbum.title == element.album);
          }
        });
        if (DeleteSongsBasedOnDifference.length != 0) {
          //Delete the song from metrics
          metricService.deleteAllMetricsOfSongs(songsToDelete);
          if (DeleteSongsBasedOnDifference.contains(
              _playerState$.value.value.uri)) {
            //refresh the playerState
            updatePlayerState(
                _playerState$.value.key,
                playback$.value.contains(Playback.shuffle)
                    ? playlist$.value.value[0]
                    : playlist$.value.key[0]);
          }
        }
      }
        ...
        newAlbumMap.keys.forEach((artistName) {
          ...
          if (newOldArtistMap[artistName] != null) {
            ...
            newAlbumMap[artistName]!.keys.forEach((newAlbumName) {
              ...
              Album tempAlbum = newOldArtistMap[artistName]!.albums.asMap().map(
                  (key, value) => MapEntry(value.title, value))[newAlbumName]!;
              //if the album is found it means we have new songs to add to that album
              if (tempAlbum != null) {
                tempAlbum.songs
                    .addAll(newAlbumMap[artistName]![newAlbumName]!.songs);
                //We also add the nexSongs to the existing album
                newOldAlbumMap[newAlbumName]!
                    .songs
                    .addAll(newAlbumMap[artistName]![newAlbumName]!.songs);
                //Sorting the new Album
                newOldAlbumMap[newAlbumName]!.songs.sort((a, b) {
                  if (a.numberInAlbum == null || b.numberInAlbum == null)
                    return 1;
                  if (a.numberInAlbum! < b.numberInAlbum!)
                    return -1;
                  else
                ...
                newOldArtistMap[artistName]!.albums.asMap().map((key, value) =>
                        MapEntry(value.title, value))[newAlbumName] =
                    newAlbumMap[artistName]![newAlbumName]!;
  ...
  void reInitializePlayStreams() {
    ...
    if (_upnpPlayerStateSubscription == null) {
      _upnpPlayerStateSubscription =
          castService.castingPlayerState.listen((data) {
          ...
          if (playerstate.value.id != null) {
            if (playerstate.key != data) {
              if (data != null && data == PlayerState.stopped) {
                playerState$
                    .add(MapEntry(PlayerState.paused, playerstate.value));
              } else {
    ...
    if (_upnpOnSongCompleteSubscription == null) {
      ...
      _upnpOnSongCompleteSubscription = Rx.combineLatest2(
          ...
          (a, b) => MapEntry<PlayerState, Duration>(a, b)).listen((data) {
        if (data != null) {
          if (data.key != null && data.value != null) {
            ...
            willCallOnSongComplete = ((data.value.inMilliseconds == 0 &&
                    (lastDuration.inMilliseconds + 2100 >
                        playerState$.value.value.duration!)) ||
  ...
  void castOrPlay(Tune song, {bool SingleCast = false, Device? device}) {
    ...
    if (currentCastingItem != null &&
        castService.castingState.value == CastState.CASTING &&
        (SingleCast == null || !SingleCast)) {
      ...
      castService.castAndPlay(song,
          SingleCast: SingleCast, deviceToUse: device);
    }
  }

  void playMusic(Tune song,
      {bool isPartOfAPlaylist = false, Playlist? playlist}) async {
    if (castService.castingState.value == CastState.CASTING) {
    ...
    if (playerstate != null) {
      if (playerstate.value != null && playerstate.value.id != null) {
        metricService.incrementPlayTimeOnSingleSong(
            playerstate.value, position$.value);
        metricService.incrementPlayTimeOnSingleArtist(
            artists$.value
                .where((element) => element.name == playerstate.value.artist)
                .toList()[0],
            position$.value);
        //Setting the playlist Time
        if (_currentPlayingPlaylist$.value.value != null &&
            _currentPlayingPlaylist$.value.value!.songs!.indexWhere((elem) {
                  return elem.id == playerstate.value.id;
                }) !=
                -1) {
          metricService.incrementPlaylistPlaytimeOnSinglePlaylist(
              _currentPlayingPlaylist$.value.value!, position$.value);
        }
      }
    }
  ...
  void playOrPause(Tune? song, {bool PlayPauseCurrentSong = false}) async {
    ...
    final bool _isSelectedSong = _currentSong.id == song!.id;
    switch (_state) {
      case PlayerState.playing:
        if (_isSelectedSong) {
  ...
  void updatePlayerState(PlayerState state, Tune song) async {
    ...
    if (!(_playerState$.value.key == state &&
        playerState$.value.value.id == song.id)) {
  ...
  Album getAlbumFromSong(Tune song) {
    return albums$.value.firstWhere((element) => element.title == song.album,
        orElse: () => throw Exception);
  }

  Artist getArtistTitle(String title) {
    return artists$.value.firstWhere((element) => element.name == title,
        orElse: () => throw Exception);
  }
  ...
  Future<Map> getSongInformation(Tune song) async {
    ...
    finalMap["Album"] = albums$.value.firstWhere(
        (element) => element.title == song.album,
        orElse: () => throw Exception);
    finalMap["playlist"] = _playlists$.value
        .where((element) =>
            element.songs!.map((e) => e.title).toList().contains(song.title))
        .toList();
  ...
  List<Tune> getMostPlayedOfArtist(Artist artist, {maxSongNumber = 11}) {
    Map<String, dynamic> mostPlayedSongs = metricService
        .getCurrentMemoryMetric(MetricIds.MET_GLOBAL_SONG_PLAY_TIME);
    ...
    playlistSongs.sort((a, b) {
      return int.parse(a.value.toString())
          .compareTo(int.parse(b.value.toString()));
    });
  ...
  void cycleBetweenPlaybackStates() {
    ...
    if ((!_playback$.value.contains(Playback.repeatSong)) &&
        (!_playback$.value.contains(Playback.repeatQueue))) {
  ...
  void _onSongComplete() {
    ...
    if (!_playback.contains(Playback.repeatQueue) &&
        !_playback.contains(Playback.repeatSong)) {
      final Tune _currentSong = _playerState$.value.value;
      final bool _isShuffle = _playback$.value.contains(Playback.shuffle);
      final List<Tune> _playlist =
          _isShuffle ? _playlist$.value.value : _playlist$.value.key;
  ...
  Future<bool>? saveSongTags(Tune newSong, Tune oldSong) {
    if (newSong != null) {
      ...
      int indexOfSongToReplace =
          newSongList.indexWhere((element) => element.id == newSong.id);
      ...
      return saveFiles(songsToSave: newSongList).then((value) {
        ...
        bool songRemoved = await metricService
            .removeSongFromLatestPlayedSongs(null, title: oldSong.title!);
        if (songRemoved) {
  ...
  Future<bool>? saveAlbumInfo(Album newAlbum, Album oldAlbum) {
    if (newAlbum != null) {
      ...
      Map<String, Tune> newSongsById = newAlbum.songs
          .asMap()
          .map((key, value) => MapEntry<String, Tune>(value.id!, value));
      //Map between the index of the newAlbum songs index and app song list
      Map<int, int> indexOfSongsToReplace = new Map();
      newSongList
          .asMap()
          .map((key, value) => MapEntry(value.id, key))
          .forEach((key, value) {
      ...
      return saveFiles(songsToSave: newSongList).then((value) {
        ...
        return Future.wait(
            newAlbum.songs.map((e) => FileService.writeTags(e)).toList());
      }).then((value) async {
        //Here we need to refresh multiple values
        //The first would be the lastSongPlayed that is saved to the storage with a separate reference
        oldAlbum.songs.forEach((element) async {
          bool songRemoved = await metricService
              .removeSongFromLatestPlayedSongs(null, title: element.title);
          if (songRemoved) {
            return metricService
                .addSongToLatestPlayedSongs(newSongsById[element.id]!);
          }
        });
  ...
  Future<bool?> getArtistDataAndSaveIt() async {
    if (SettingsService.settings$.value[SettingsIds.SET_ARTIST_THUMB_UPDATE] ==
        "true") {
      if (_artists$.value.isNotEmpty) {
            ...
            queueService.addItemsToQueue(QueueItem(
                ...
                execute: () async {
                  ...
                  if (SettingsService.settings$
                          .value[SettingsIds.SET_ARTIST_THUMB_UPDATE] ==
                      "true") {
                    Artist artist = await artistThumbRetreival(elem);
                    List<int> colors =
                        await themeService.getArtistColors(artist);
                    elem.colors = colors;
        ...
        queueService.setOnQueueEnd(() {
          SettingsService.updateSingleSetting(
              SettingsIds.SET_ARTIST_THUMB_UPDATE, "false");
        });
  ...
  Future<Artist> artistThumbRetreival(Artist artist) async {
    ...
    if (data != null && data["images"] != null && data["images"].length != 0) {
      ...
      if (imagetOUserMap != null) {
        String imagetOUser = "";
        String ThumbQualitySetting = SettingsService.getCurrentMemorySetting(
  ...
  void _initStreams() {
    ...
    _currentPlayingPlaylist$ =
        BehaviorSubject<MapEntry<PlayerState, Playlist?>>.seeded(
    ...
    artists$.listen((value) {
      ...
      ArtistList = Map.fromIterable(artists$.value,
          key: (keyvalue) => keyvalue.id, value: (value) => value);
    });
    SongList = new Map();
    _songs$.listen((value) {
