...
class StandardIsolateFunctions {
  ...
  static Future<bool> readExtDir(
      Directory dir, Function(String) callback) async {
  ...
  static saveSongsToPref(
      List<Tune> songs, Function(List<String>) callback) async {
    ...
    ItemsList.forEach((Album album) {
      if (artists["${album.artist}"] != null) {
        ...
        artists["${album.artist}"] =
            Artist(currentIndex, album.artist, null, null);
        artists["${album.artist}"]!.albums.add(album);
        currentIndex++;
      }
    });
  ...
  static Future<List<Album>> fetchAlbumFromsongs(List<Tune> songs,
      {Function(List<Album>)? callback}) async {
  ...
  static saveArtiststoPref(
      List<Artist> artists, Function(List<String>) callback) async {
    List<String> _encodedStrings = [];
  ...
  static Map? getTopAlbum(Map<String, dynamic> GlobalSongPlayTime,
      Map<int, Album> albums, Map<String, Tune> songs, Function(Map) callback) {
    ...
    topAlbums
        .removeWhere((element) => element == null || element.title == null);
    Map<String, Duration> playDuration = playTime.map((k1, V1) {
      return MapEntry(k1, Duration(seconds: V1));
    });
    if (callback != null) {
      callback({"topAlbums": topAlbums, "playDuration": playDuration});
    }
    return null;
  }

  ///
  static Map<String, dynamic>? getMostPlayedSongs(
      Map<String, dynamic> metricValues,
      Map<int?, Artist?>? artists,
      Map<String, Tune> songs,
    ...
    if (newValue.length != 0) {
      ...
      newSongMap.keys.toList().forEach((element) {
        if (element != null)
          artistsAndTheirPresenceInMostPlayed![element.artist!] =
              (artistsAndTheirPresenceInMostPlayed[element.artist] != null
                  ? artistsAndTheirPresenceInMostPlayed[element.artist!] =
                      artistsAndTheirPresenceInMostPlayed[element.artist!]! + 1
                  : 1);
      ...
      if (newSongMap.length < 10) {
        ...
        for (int i = 0; i < 10 - newSongMap.length; i++) {
          ...
          Map<String, int> sortedPresenceMap = new Map.fromIterable(
              sortedPresenceKeys,
              key: (k) => k,
              value: (k) => int.parse(newValue[k]));
          ...
          String nameOfArtistsToPickFrom =
              sortedPresenceMap.keys.toList()[indexOfArtistWithPriority];
          while (sortedPresenceMap[nameOfArtistsToPickFrom]! < 2 &&
              indexOfArtistWithPriority <
                  sortedPresenceMap.keys.toList().length) {
            nameOfArtistsToPickFrom =
                sortedPresenceMap.keys.toList()[indexOfArtistWithPriority];
            indexOfArtistWithPriority++;
          }
          Artist? artistToPickFrom = artists!.values.firstWhere(
              (element) => element!.name == nameOfArtistsToPickFrom,
              orElse: () => null);
          ...
          int albumIndex =
              MathUtils.getRandomFromRange(0, artistToPickFrom.albums.length);
          int songIndex = MathUtils.getRandomFromRange(
              0, artistToPickFrom.albums[albumIndex].songs.length);
          ...
          newSongMap[artistToPickFrom.albums[albumIndex].songs[songIndex]] =
              newSongMap.values.toList().last;
          artistsAndTheirPresenceInMostPlayed[artistToPickFrom.name!] =
              artistsAndTheirPresenceInMostPlayed[artistToPickFrom.name!]! + 1;
      ...
      mostPlayedSongsToReturn = newSongMap.keys.toList() as List<Tune>;
      mostPlayedSongsToReturn.removeWhere((element) => element == null);
    }

    discoverableArtists = _getDiscoverableArtistsFromTpArtists(
        topArtists, artists as Map<int, Artist>);
  ...
  static List<Artist> _getDiscoverableArtistsFromTpArtists(
      List<Artist> topArtists, Map<int, Artist> allArtists) {
    List<int> topArtistsIds = topArtists.map((e) => e.id).toList() as List<int>;
    ...
    for (int i = 0; i < 4; i++) {
      randomIndexList
          .add(MathUtils.getRandomFromRange(0, notTopArtists.length));
    }
    return randomIndexList.map((e) => notTopArtists[e]).toList();
  }

  static void createServerAndAddImagesAndFiles(
      String ip, String port, Function(dynamic) callback) {
              ...
              file.exists().then((bool found) async {
                if (found) {
                  request.response.headers.contentType = ContentType(
                      contentType[0] ?? "audio", contentType[1] ?? "mpeg");
