...
class ThemeService {
  ...
  void updateTheme(Tune song) async {
    final color = await MessagingUtils.sendNewIsolateCommand(
        command: "updateTheme",
        message: {"songId": song.id, "coverArt": song.albumArt});
  ...
  Future<List<int>> getArtistColors(Artist artist) async {
    final color = await MessagingUtils.sendNewIsolateCommand(
        command: "getArtistColors",
        message: {"artistId": artist.id, "coverArt": artist.coverArt});
