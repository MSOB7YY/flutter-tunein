...
import 'package:flutter_tunein_attempt3/components/albumSongList.dart';
...
import 'package:flutter_tunein_attempt3/components/smallControlls.dart';
...
class _playingQueueState extends State<playingQueue>
    with AutomaticKeepAliveClientMixin<playingQueue> {
  ...
  BehaviorSubject<MapEntry<List<Playback>, MapEntry<List<Tune>, List<Tune>>>>
      newStream = new BehaviorSubject<
          MapEntry<List<Playback>, MapEntry<List<Tune>, List<Tune>>>>();
  ...
  void initState() {
    ...
    Stream<MapEntry<List<Playback>, MapEntry<List<Tune>, List<Tune>>>>
        originalStream = Rx.combineLatest2(
  ...
  Widget build(BuildContext context) {
    ...
    Future<bool> animate(
        int indexOfThePlayingSong, double numberOfSongsPerScreen) async {
      if (this.controller!.hasClients) {
        print("will animate");
        await this.controller!.animateTo(
            ((indexOfThePlayingSong + 1) * 62) +
                getSongPosition(indexOfThePlayingSong, numberOfSongsPerScreen),
            duration: Duration(
                milliseconds: (pow(
                            log((indexOfThePlayingSong > 0
                                    ? indexOfThePlayingSong
                                    : 1) *
                                2),
                            2))
                        .floor() +
    ...
    WidgetsBinding.instance.addPostFrameCallback((duration) {
      ...
      scrollAnimationListener = musicService.playerState$
          .listen((MapEntry<PlayerState, Tune> value) async {
        if (value != null && songs != null) {
          int indexOfThePlayingSong =
              songs!.indexWhere((elem) => elem.id == value.value.id);
          if (indexOfThePlayingSong > 0) {
            bool didanimate =
                await animate(indexOfThePlayingSong, numberOfSongsPerScreen);
            if (didanimate == true) {
    ...
    return StreamBuilder<
      ...
      builder: (BuildContext context,
        ...
        return StreamBuilder(
          ...
          builder: (BuildContext context,
              AsyncSnapshot<MapEntry<PlayerState, Tune>> snapshotOne) {
            ...
            return StreamBuilder(
              stream: themeService
                  .getThemeColors(
                      _currentSong != null ? _currentSong : null as Tune)
                  .asStream(),
              builder:
                ...
                return Container(
                  color:
                      bgColor != null ? Color(bgColor[0]) : MyTheme.darkBlack,
                  child: new Column(
                    children: <Widget>[
                      Material(
                          ...
                          child: Row(
                            ...
                            children: <Widget>[
                                  ...
                                  child: ListView.builder(
                                    ...
                                    itemBuilder: (context, index) {
                                      ...
                                      return MyCard(
                                        ...
                                        onTap: () {
                                          musicService
                                              .playOrPause(_playlist[newIndex]);
                                        },
                                      );
                                    },
                                  ),
                              ...
                              MyScrollbar(
                                controller: controller,
                                color:
                                    bgColor != null ? Color(bgColor[0]) : null,
                              ),
                            ],
                          ),
