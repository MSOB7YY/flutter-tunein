  ...
  Widget build(BuildContext context) {
    ...
    Future<bool> animate(int indexOfThePlayingSong, double numberOfSongsPerScreen) async {
      if (this.controller!.hasClients) {
        print("will animate");
        await this.controller!.animateTo(((indexOfThePlayingSong + 1) * 62) + getSongPosition(indexOfThePlayingSong, numberOfSongsPerScreen),
            duration: Duration(milliseconds: (pow(log((indexOfThePlayingSong > 0 ? indexOfThePlayingSong : 1) * 2), 2)).floor() + 50), curve: Curves.fastOutSlowIn);
        return true;
      } else {
    ...
    WidgetsBinding.instance.addPostFrameCallback((duration) {
      ...
      //BehaviorSubject<MapEntry<PlayerState, Tune?>>
      //StreamSubscription<MapEntry<PlayerState, Tune?>>?
    ...
    return StreamBuilder<MapEntry<List<Playback>, MapEntry<List<Tune>?, List<Tune>?>>>(
      ...
      builder: (BuildContext context, AsyncSnapshot<MapEntry<List<Playback>, MapEntry<List<Tune>?, List<Tune>?>>> snapshot) {
        ...
        final List<Tune> _playlist = (_isShuffle ? snapshot.data!.value.value : snapshot.data!.value.key) ?? [];
        songs = _playlist;
        ...
        return StreamBuilder(
          ...
          builder: (BuildContext context, AsyncSnapshot<MapEntry<PlayerState, Tune?>> snapshotOne) {
            ...
            return StreamBuilder(
              stream: themeService.getThemeColors(_currentSong != null ? _currentSong : null as Tune).asStream(),
                                ...
                                child: Row(
                                  ...
                                  children: <Widget>[
                                    ...
                                    Expanded(
                                      flex: 7,
                                      child: Container(
                                        margin: EdgeInsets.all(8).subtract(EdgeInsets.only(left: 8)).add(EdgeInsets.only(
                                            top: ((_currentSong?.title?.length ?? 0) > 27 ? _currentSong!.title!.length / (_currentSong.title!.length - 12) : 10).toDouble()
                                        )
                                        ),
                                        child: Column(
                                          ...
                                          children: <Widget>[
                                            Padding(
                                              ...
                                              child: Text(
                                                _currentSong?.title ?? "Unknon Title" ,
                                            ...
                                            Container(
                                              ...
                                              child: Row(
                                                ...
                                                children: <Widget>[
                                                  Container(
                                                    child: Text(
                                                      "${Duration(milliseconds: _currentSong?.duration ?? 0).inMinutes} min",
                                                      style: TextStyle(
                                                        color: bgColor != null ? Color(bgColor[2]) : Colors.white70,
                                                        fontWeight: FontWeight.w700,
                                                        fontSize: 14,
                                            ...
                                            case 7:
                                              {
                                                DialogService.showAlertDialog(context,
                                                    title: "Song Information", content: SongInfoWidget(null, song: _playlist[newIndex]), padding: EdgeInsets.only(top: 10));
                                                break;
                                              }
                                            case 8:
                                              {
