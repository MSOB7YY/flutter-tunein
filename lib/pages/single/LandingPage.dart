...
class _LandingPageState extends State<LandingPage> with AutomaticKeepAliveClientMixin<LandingPage> {
  ...
  Future<dynamic> getMostPlayedSongs(Map<String, dynamic> metricValues) {
    ...
    MusicServiceIsolate.sendCrossIsolateMessage(
        CrossIsolatesMessage(sender: tempPort.sendPort, command: "getMostPlayedSongs", message: [metricValues, musicService.ArtistList, musicService.SongList]));
  ...
  Future<dynamic> getTopAlbum(Map<String, dynamic> GlobalSongPlayTime) {
    ReceivePort tempPort = ReceivePort();
    MusicServiceIsolate.sendCrossIsolateMessage(
        CrossIsolatesMessage(sender: tempPort.sendPort, command: "getTopAlbums", message: [GlobalSongPlayTime, musicService.AlbumList, musicService.SongList]));
    return (tempPort.firstWhere((event) {
      return event != "OK";
    }));
  }
                    ...
                    builder: (context, AsyncSnapshot<dynamic> snapshot) {
                      Widget shallowWidget = Container(
                        ...
                        child: Center(
                          child: Text(
                            "Looking for Albums",
                            style: TextStyle(color: MyTheme.grey300.withOpacity(.8), fontWeight: FontWeight.w600, fontSize: 18),
                          ),
                        ),
                      );
                      if (!snapshot.hasData) {
                      ...
                      Map<String, Duration>? PlayDuration = albumData["playDuration"];
                ...
                SliverToBoxAdapter(
                  child: Container(
                    ...
                    child: ListView(
                      ...
                      shrinkWrap: false,
                      itemExtent: 500,
                      physics: StageScrollingPhysics(currentStage: 0, stages: [0, (screenSize.width / 4) + 40]),
                      cacheExtent: 122,
                                                ...
                                                children: currentPlaylist.map((e) {
                                                  ...
                                                  return ListTile(
                                                    ...
                                                    leading: isCurrentSong
                                                        ? Icon(isPlaying ? Icons.pause_circle_outline : Icons.play_circle_outline, size: 28, color: MyTheme.darkRed)
                                                        : Icon(Icons.play_circle_outline, size: 28, color: MyTheme.darkgrey),
                                                    title: Text(
                                                      e.title!,
                                                      maxLines: 1,
                                                      strutStyle: StrutStyle(height: 1, forceStrutHeight: true),
                                                      style: TextStyle(color: MyTheme.grey300, fontWeight: FontWeight.w700, fontSize: 15),
                                                    ),
  ...
  Widget getMostPlayedWidget(context, Map<String, int>? artistPresence, List<Tune>? mostPlayedSongs) {
    ...
    return Material(
        color: Colors.transparent,
        child: StreamBuilder(
          stream: (artistToPutToWidgetBackground?.isNotEmpty ?? false) ? backgroundimagesForMostPlayedSongs.asStream() : Future.wait([Asset8bitList]).asStream(),
                ...
                showGeneralDialog(
                    ...
                    pageBuilder: (context, anim1, anim2) {
                      return MultipleSongTapPopupWidget(
                          context: context,
                          TopBackgroundWidget:
                              getCombinedImages(snapshot.data!, standardHeight: screensize.height * 0.2, standardWidth: screensize.width * 0.85, maxWidth: screensize.width * 0.85),
                          screensize: screensize,
                          onShuffleButtonTap: () {
                            musicService.updatePlaylist(mostPlayedSongs);
                            musicService.updatePlayback(Playback.shuffle);
                            musicService.stopMusic();
                            musicService.playMusic(musicService.playlist$.value.value?.elementAt(0));
                          },
                          ...
                          onPlaybuttonTap: () {
                            musicService.updatePlaylist(mostPlayedSongs);
                            musicService.stopMusic();
                            musicService.playMusic(mostPlayedSongs?.elementAt(0));
                          },
                          onSaveButtonTap: () async {
                            bool result = await DialogService.showConfirmDialog(context, message: "Save the most played songs as a playlist", title: "Save as a Playlist");
                            if (result) {
                              ...
                              Playlist newPlaylist = new Playlist("Most Played ${DateTime.now().toIso8601String()}", mostPlayedSongs, PlayerState.stopped, fileURI.path);

                              musicService.addPlaylist(newPlaylist).then((value) {
                                deckItemStateStream!["save"]!
                                    .add({"withBadge": false, "badgeContent": null, "badgeColor": null, "iconColor": MyTheme.darkRed, "icon": null, "title": "Saved !"});
                          ...
                          showGeneralDialog(
                              ...
                              pageBuilder: (context, anim1, anim2) {
                                return MultipleSongTapPopupWidget(
                                  ...
                                  onSaveButtonTap: () async {
                                    bool result = await DialogService.showConfirmDialog(context, message: "Save this songs list as a playlist", title: "Save as a Playlist");
                                    if (result) {
                                      deckItemStateStream!["save"]!.add({
                                        "withBadge": true,
                                        "badgeContent": Icon(
                                      ...
                                      Uri fileURI = await FileService.saveBytesToFile(topImage!);
                                      Playlist newPlaylist = new Playlist("Random Songs Playlist ${DateTime.now().toIso8601String()}", songsToChooseFrom, PlayerState.stopped, fileURI.path);
          ...
          child: Container(
            ...
            child: ListView.builder(
              itemBuilder: (context, index) {
                return Material(
                    child: StreamBuilder(
                      stream: AlbumSongs[index].albumArt != null ? ConversionUtils.FileUriTo8Bit(AlbumSongs[index].albumArt!, fileInstead: File("")).asStream() : Asset8bitList.asStream(),
                      builder: (context, AsyncSnapshot<List<int>> snapshot) {
                        ...
                        if (!snapshot.hasData) {
                          return Container(decoration: BoxDecoration(color: MyTheme.bgBottomBar));
                        }
                        return GestureDetector(
                          child: Container(
                            margin: EdgeInsets.only(right: 8),
                            child: ShowWithFade.fromStream(
                              ...
                              shallowWidget: Container(decoration: BoxDecoration(color: MyTheme.bgBottomBar)),
                            ),
                          ),
                          onTap: () {
                            List<int> colors = AlbumSongs[index].songs[0].colors as List<int>;
                            showGeneralDialog(
          ...
          child: Container(
            ...
            child: ListView.builder(
              itemBuilder: (context, index) {
                return Material(
                    child: StreamBuilder(
                      stream: Artists[index].coverArt != null ? ConversionUtils.FileUriTo8Bit(Artists[index].coverArt!, fileInstead: File("")).asStream() : Asset8bitList.asStream(),
                      builder: (context, AsyncSnapshot<List<int>> snapshot) {
                        if (snapshot.hasError) {
                          return PreferredPicks(
                            allImageBlur: false,
              ...
              child: GenericSongList(
                ...
                onContextOptionSelect: (choice, tune) async {
                  switch (choice.id) {
                    ...
                    case 7:
                      {
                        DialogService.showAlertDialog(context, title: "Song Information", content: SongInfoWidget(null, song: tune), padding: EdgeInsets.only(top: 10));
                        break;
                      }
                    case 8:
                      {
                            ...
                            child: TopLeftWidget ??
                                FadeInImage(
                                  ...
                                  image: (imageFile != null && imageFile.existsSync())
                                      ...
                                      : AssetImage('images/cover.png') as ImageProvider<Object>,
                                )),
                        Container(
                            ...
                            children: <Widget>[
                              Padding(
                                child: Text(
                                  ...
                                  textAlign: TextAlign.center,
                                  maxLines: 1,
                                  style: TextStyle(color: colors != null ? Color(colors[1]) : MyTheme.grey300, fontWeight: FontWeight.w600, fontSize: 17),
                                ),
                              ...
                              Padding(
                                ...
                                child: Text(
                                  ...
                                  maxLines: 1,
                                  style: TextStyle(color: (colors != null ? Color(colors[1]) : MyTheme.grey300).withOpacity(.8), fontWeight: FontWeight.w500, fontSize: 15),
                                ),
                              ),
                              underSubtitleTray ??
                                  Row(
        ...
        child: Column(
          ...
          children: <Widget>[
                            ...
                            children: <Widget>[
                              Padding(
                                child: Text(
                                  ...
                                  textAlign: TextAlign.center,
                                  maxLines: 1,
                                  style: TextStyle(color: (colors != null && colors.length != 0) ? Color(colors[1]) : MyTheme.grey300, fontWeight: FontWeight.w600, fontSize: 17),
                                ),
                                padding: EdgeInsets.only(bottom: 5),
                              ),
                              Padding(
                                ...
                                child: Text(
                                  ...
                                  maxLines: 1,
                                  style: TextStyle(
                                      color: ((colors != null && colors.length != 0) ? Color(colors[1]) : MyTheme.grey300).withOpacity(.8), fontWeight: FontWeight.w500, fontSize: 15),
                                ),
                              ),
            ...
            ShowWithFade(
              ...
              child: Container(
                decoration: BoxDecoration(borderRadius: BorderRadius.only(bottomLeft: Radius.circular(10), bottomRight: Radius.circular(10)), color: MyTheme.darkBlack),
                width: screensize.width * 0.85,
                height: screensize.height * 0.5,
