...
import 'package:flutter_tunein_attempt3/components/cards/AnimatedDialog.dart';
import 'package:flutter_tunein_attempt3/components/cards/PreferedPicks.dart';
import 'package:flutter_tunein_attempt3/components/cards/expandableItems.dart';
import 'package:flutter_tunein_attempt3/components/common/ShowWithFadeComponent.dart';
import 'package:flutter_tunein_attempt3/components/genericSongList.dart';
import 'package:flutter_tunein_attempt3/components/itemListDevider.dart';
import 'package:flutter_tunein_attempt3/components/songInfoWidget.dart';
import 'package:flutter_tunein_attempt3/components/stageScrollingPhysics.dart';
import 'package:flutter_tunein_attempt3/components/trackListDeck.dart';
import 'package:flutter_tunein_attempt3/components/trackListDeckItem.dart';
...
import 'package:flutter_tunein_attempt3/plugins/upnp.dart';
...
import 'package:flutter_tunein_attempt3/services/routes/pageRoutes.dart';
import 'package:flutter_tunein_attempt3/services/uiScaleService.dart';
import 'package:flutter_tunein_attempt3/utils/MathUtils.dart';
...
import 'package:flutter_tunein_attempt3/utils/ConversionUtils.dart';
...
class _LandingPageState extends State<LandingPage>
  ...
  void initState() {
    ...
    deckItemStateStream = {"save": BehaviorSubject<Map<String, dynamic>>()};
  ...
  Widget getCombinedImages(List<List<int>> image8BitList,
      {double standardWidth = 400,
      double standardHeight = 400,
      double maxWidth = 400}) {
  ...
  Widget build(BuildContext context) {
    ...
    topAlbumsStream.addStream(metricService
        .getOrCreateSingleSettingStream(MetricIds.MET_GLOBAL_SONG_PLAY_TIME)
        .asyncMap(((value) => getTopAlbum(value))));
    BehaviorSubject<dynamic> mostPlayedStream = new BehaviorSubject<dynamic>();
    mostPlayedStream.addStream(metricService
        .getOrCreateSingleSettingStream(MetricIds.MET_GLOBAL_SONG_PLAY_TIME)
        .asyncMap((value) => getMostPlayedSongs(value)));
    return Container(
                    ...
                    builder: (context, AsyncSnapshot<dynamic> msnapshot) {
                      if (!msnapshot.hasData) {
                        return Container(
                          ...
                          child: PreferredPicks(
                            ...
                            colors: [
                              MyTheme.bgBottomBar.value,
                              MyTheme.darkBlack.value
                            ],
                          ),
                        );
                      }
                      Map<String, int> artistPresence =
                          msnapshot.data["artistsPresence"];
                      List<Tune> mostPlayedSongs =
                          msnapshot.data["mostPlayedSongs"];
                      ...
                      if (mostPlayedSongs == null || mostPlayedSongs == null) {
                        MostPlayedWiget = Container(
                          ...
                          child: Center(
                            child: Text(
                              "Not enough songs to process most played",
                              style: TextStyle(
                        ...
                        MostPlayedWiget = getMostPlayedWidget(
                            context, artistPresence, mostPlayedSongs);
                      }
                      int firstLimiter = MathUtils.getRandomFromRange(
                          0, musicService.songs$.value.length - 10);
                      List<Tune> randomSongs = musicService.songs$.value
                          .sublist(firstLimiter, firstLimiter + 10);
                      Widget RandomSongsWidgets =
                          getRandomSongsWidget(context, randomSongs);
                      return Container(
                        height: 150,
                        child: ListView(
                          children: <Widget>[
                            if (MostPlayedWiget != null) MostPlayedWiget,
                            if (RandomSongsWidgets != null)
                              StreamBuilder(
                                ...
                                builder: (context, s) {
                                  ...
                                  int firstLimiter =
                                      MathUtils.getRandomFromRange(
                                          0,
                                          musicService.songs$.value.length -
                                              10);
                                  List<Tune> randomSongs = musicService
                                      .songs$.value
                                      .sublist(firstLimiter, firstLimiter + 10);
                                  return getRandomSongsWidget(
                                      context, randomSongs);
                                },
                              )
                          ],
                ...
                SliverToBoxAdapter(
                  child: StreamBuilder(
                    ...
                    builder: (context, AsyncSnapshot<dynamic> snapshot) {
                      ...
                      Map<String, Duration> PlayDuration =
                          albumData["playDuration"];
                      ...
                      if (topAlbums == null) {
                        return Container(
                          ...
                          child: Center(
                            child: Text(
                              "You didn't listen to enough albums",
                              style: TextStyle(
                                  color: MyTheme.grey300.withOpacity(.8),
                                  fontWeight: FontWeight.w600,
                                  fontSize: 18),
                      ...
                      return getTopAlbumsWidget(
                          context, topAlbums, PlayDuration);
                    },
                  ),
                ),
                SliverToBoxAdapter(
                                ...
                                builder:
                                  ...
                                  FixedExtentScrollController controller =
                                      new FixedExtentScrollController(
                                          initialItem: musicService
                                              .getSongIndex(currentSong));
                                  PlayerState currentState =
                                      snapshot.data![0].key;
                                  bool isPlaying =
                                      currentState == PlayerState.playing;
                                  final bool _isShuffle = snapshot.data![2]
                                      .contains(Playback.shuffle);
                                  final List<Tune> currentPlaylist = _isShuffle
                  ...
                  child: StreamBuilder(
                    ...
                    builder: (context, AsyncSnapshot<dynamic> snapshot) {
                      ...
                      List<Artist> discoverableArtists =
                          snapshot.data["discoverableArtists"];
                      Map<String, int> playDuration =
                          snapshot.data["playDuration"];

                      if (discoverableArtists == null ||
                          discoverableArtists.length == 0) {
                        return Container(
                          height: 190,
                          color: MyTheme.darkBlack,
                          padding: EdgeInsets.only(top: 10, bottom: 10),
                      ...
                      return getDiscoverArtistWidget(
                          context, discoverableArtists, playDuration);
                    },
                  ),
                )
              ],
  ...
  Widget getMostPlayedWidget(
      context, Map<String, int> artistPresence, List<Tune> mostPlayedSongs) {
    Size screensize = MediaQuery.of(context).size;
    ...
    mostPlayedSongs =
        mostPlayedSongs.sublist(0, min(9, mostPlayedSongs.length));
    List<Artist> artistToPutToWidgetBackground = artistPresence.keys
        .toList()
        .sublist(0, min(4, artistPresence.length))
        .map((e) {
      return musicService.artists$.value
          .firstWhere((element) => element.name == e);
    }).toList();
    Future<List<List<int>>> backgroundimagesForMostPlayedSongs =
        Future.wait(artistToPutToWidgetBackground.map((e) async {
      if (e.coverArt == null) {
        return await Asset8bitList;
      }
      return await ConversionUtils.FileUriTo8Bit(
        e.coverArt!,
        fileInstead: File.fromUri(Uri()),
      );
    }).toList());
    Uint8List? topImage;
    return Material(
        child: StreamBuilder(
          stream: artistToPutToWidgetBackground.length != 0
              ? backgroundimagesForMostPlayedSongs.asStream()
              : Future.wait([Asset8bitList]).asStream(),
                  ...
                  inStream: ConversionUtils.createImageFromWidget(
                          Directionality(
                            ...
                            child: PreferredPicks(
                              ...
                              allImageBlur: false,
                              colors: [
                                MyTheme.grey300.value,
                                MyTheme.darkBlack.value
                              ],
                              backgroundWidget: getCombinedImages(
                                  snapshot.data!,
                                  standardHeight: 150,
                                  standardWidth: 200,
                                  maxWidth: 200),
                ...
                showGeneralDialog(
                    ...
                    pageBuilder: (context, anim1, anim2) {
                      return MultipleSongTapPopupWidget(
                          context: context,
                          TopBackgroundWidget: getCombinedImages(snapshot.data!,
                              standardHeight: screensize.height * 0.2,
                              standardWidth: screensize.width * 0.85,
                          ...
                          onShuffleButtonTap: () {
                            ...
                            musicService.playMusic(
                                musicService.playlist$.value.value[0]);
                          },
                          ...
                          onSaveButtonTap: () async {
                            bool result = await DialogService.showConfirmDialog(
                                context,
                                message:
                                    "Save the most played songs as a playlist",
                            ...
                            if (result) {
                              ...
                              Uri fileURI =
                                  await FileService.saveBytesToFile(topImage!);
  ...
  Widget getRandomSongsWidget(context, List<Tune> songsToChooseFrom) {
    if (songsToChooseFrom != null && songsToChooseFrom.length != 0) {
      ...
      int firstBracket = MathUtils.getRandomFromRange(
          0,
          (min(
              (songsToChooseFrom.length - 4).abs(), songsToChooseFrom.length)));

      List<Tune> backgroundIamgeSongs = songsToChooseFrom.sublist(firstBracket,
          firstBracket + min(4, songsToChooseFrom.length - firstBracket));
      Future<List<List<int>>> backgroundimagesForMostPlayedSongs =
          Future.wait(backgroundIamgeSongs.map((e) async {
        if (e.albumArt == null) {
          return await Asset8bitList;
        }
      ...
      return Material(
          child: StreamBuilder(
            stream: backgroundIamgeSongs.length != 0
                ? backgroundimagesForMostPlayedSongs.asStream()
                : Future.wait([Asset8bitList]).asStream(),
            builder: (context, AsyncSnapshot<List<List<int>>> snapshot) {
                          ...
                          showGeneralDialog(
                              ...
                              pageBuilder: (context, anim1, anim2) {
                                return MultipleSongTapPopupWidget(
                                  ...
                                  onPlaybuttonTap: () {
                                    musicService
                                        .updatePlaylist(songsToChooseFrom);
                                    musicService.stopMusic();
                                    musicService
                                        .playMusic(songsToChooseFrom[0]);
                                  },
                                  onSaveButtonTap: () async {
                                    bool result =
                                        await DialogService.showConfirmDialog(
                                            context,
                                            message:
                                                "Save this songs list as a playlist",
                                            title: "Save as a Playlist");
                                    if (result) {
                                      deckItemStateStream!["save"]!.add({
                                        "withBadge": true,
                                      ...
                                      Uri fileURI =
                                          await FileService.saveBytesToFile(
                                              topImage!);
                                      Playlist newPlaylist = new Playlist(
                                  ...
                                  screensize: screensize,
                                  TopBackgroundWidget: getCombinedImages(
                                      snapshot.data!,
                                      standardHeight: screensize.height * 0.2,
                                      standardWidth: screensize.width * 0.85,
                                      maxWidth: screensize.width * 0.85),
  ...
  Widget getTopAlbumsWidget(
      context, List<Album> AlbumSongs, Map<String, Duration> playDuration) {
          ...
          child: Container(
            ...
            child: ListView.builder(
              itemBuilder: (context, index) {
                return Material(
                    child: StreamBuilder(
                      stream: AlbumSongs[index].albumArt != null
                          ? ConversionUtils.FileUriTo8Bit(
                                  AlbumSongs[index].albumArt!,
                                  fileInstead: File(""))
                              .asStream()
                          : Asset8bitList.asStream(),
                      builder: (context, AsyncSnapshot<List<int>> snapshot) {
                        ...
                        if (!snapshot.hasData) {
                          return Container(
                              decoration:
                                  BoxDecoration(color: MyTheme.bgBottomBar));
                        }
                        return GestureDetector(
                          child: Container(
                            margin: EdgeInsets.only(right: 8),
                              ...
                              inStream: ConversionUtils.createImageFromWidget(
                                      Directionality(
                                          ...
                                          child: PreferredPicks(
                                            allImageBlur: false,
                                            bottomTitle:
                                                "${AlbumSongs[index].title!.split(' ').join('\n')}",
                                            backgroundWidget: getCombinedImages(
                                                [snapshot.data!],
                                                maxWidth: 122,
                                                standardWidth: 122,
                                                standardHeight: 190),
                                            colors: AlbumSongs[index]
                                                .songs[0]
                                                .colors!
                                                .map((e) {
                                              return Color(e!)
                          ...
                          onTap: () {
                            List<int> colors =
                                AlbumSongs[index].songs[0].colors as List<int>;
                            showGeneralDialog(
                                  ...
                                  return SinglePicturePopupWidget(
                                      ...
                                      onPlaybuttonTap: () {
                                        musicService.updatePlaylist(
                                            AlbumSongs[index].songs);
                                        musicService.stopMusic();
                                        musicService.playMusic(
                                            AlbumSongs[index].songs[0]);
                                      },
                                      onShuffleButtonTap: () {
                                        musicService.updatePlaylist(
                                            AlbumSongs[index].songs);
                                        musicService
                                            .updatePlayback(Playback.shuffle);
                                        musicService.stopMusic();
                                        musicService.playMusic(musicService
  ...
  Widget getDiscoverArtistWidget(
      context, List<Artist> Artists, Map<String, int> playDuration) {
          ...
          child: Container(
            ...
            child: ListView.builder(
              itemBuilder: (context, index) {
                return Material(
                    child: StreamBuilder(
                      stream: Artists[index].coverArt != null
                          ? ConversionUtils.FileUriTo8Bit(
                                  Artists[index].coverArt!,
                                  fileInstead: File(""))
                              .asStream()
                          : Asset8bitList.asStream(),
                      builder: (context, AsyncSnapshot<List<int>> snapshot) {
          ...
          GestureDetector(
            ...
            child: ShowWithFade.fromStream(
              inStream: ConversionUtils.createImageFromWidget(
                      ...
                      imageSize: Size((screensize.width * 0.85) + 10,
                          screensize.height * 0.2),
                      logicalSize: Size((screensize.width * 0.85) + 10,
                          screensize.height * 0.2),
              ...
              child: GenericSongList(
                ...
                onContextOptionSelect: (choice, tune) async {
                  switch (choice.id) {
                    ...
                    case 5:
                      {
                        if (castService.currentDeviceToBeUsed.value == null) {
                          upnp.Device result =
                              await DialogService.openDevicePickingDialog(
                                  context, null);
                          if (result != null) {
                    ...
                    case 10:
                      {
                        PageRoutes.goToEditTagsPage(tune, context,
  ...
  SinglePictureArtistPopupWidget(
    ...
    File imageFile = TopLeftWidget == null
        ...
        : TopLeftWidget as File;
    double popupWidth = screensize!.width * 0.85;
    double albumGridCellHeight = uiScaleService.AlbumArtistInfoPage(
        Size(popupWidth, screensize.height * 0.75));
    int itemsPerRow = 3;
              ...
              child: Container(
                ...
                child: GridView.builder(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    ...
                    return GestureDetector(
                      onTap: () {
                        PageRoutes.goToAlbumSongsList(null, context,
                      ...
                      child: AlbumGridCell(
                        ...
                        animationDelay: (80 * newIndex) -
                            (index < 3 ? ((3 - index) * 150) : 0),
                        useAnimation: !(80 == 0),
                        ...
                        onContextSelect: (choice) {
                          switch (choice.id) {
                            case 1:
                              {
                                musicService
                                    .playEntireAlbum(artist.albums[index]);
                                break;
                              }
                            case 2:
                              {
                                musicService
                                    .shuffleEntireAlbum(artist.albums[index]);
                                break;
                              }
                          }
                        },
