...
class _TrackListDeckItemState extends State<TrackListDeckItem> {
  ...
  void initState() {
    ...
    if (initialState?.extraData != null) {
      withBadge ?? initialState?.extraData!["withBadge"] != null
          ? withBadge = initialState?.extraData!["withBadge"]
          : null;
      title ??
          (initialState?.extraData!["title"] != null
              ? title = initialState?.extraData!["title"]
              : null);
      subtitle ??
          (initialState?.extraData!["subtitle"] != null
              ? subtitle = initialState?.extraData!["subtitle"]
              : null);
      iconColor ??
          (initialState?.extraData!["iconColor"] != null
              ? iconColor =
                  Color(int.parse(initialState?.extraData!["iconColor"]))
              : null);
      titleColor ??
          (initialState?.extraData!["titleColor"] != null
              ? titleColor =
                  Color(int.parse(initialState?.extraData!["titleColor"]))
              : null);
      subtitleColor ??
          (initialState?.extraData!["subtitleColor"] != null
              ? subtitleColor =
                  Color(int.parse(initialState?.extraData!["subtitleColor"]))
              : null);
      badgeColor ??
          (initialState?.extraData!["badgeColor"] != null
              ? badgeColor =
                  Color(int.parse(initialState?.extraData!["badgeColor"]))
              : null);
    }
    if (widget.onBuild != null) {
      Future.value(widget.onBuild!()).then((data) {
        Map? ret = data as Map<dynamic, dynamic>?;
        if (ret != null) {
          ret["withBadge"] != null ? withBadge = ret["withBadge"] : null;
          ret["badgeContent"] != null
              ? currentBadge = ret["badgeContent"]
              : null;
          ...
          ret["subtitleColor"] != null
              ? subtitleColor = ret["subtitleColor"]
              : null;
          ...
          if (onTap != null) {
            Future.value(onTap!()).then((data) {
              ...
              if (ret != null) {
                ...
                ret["iconColor"] != null ? iconColor = ret["iconColor"] : null;
                ret["titleColor"] != null
                    ? titleColor = ret["titleColor"]
                    : null;
                ret["subtitleColor"] != null
                    ? subtitleColor = ret["subtitleColor"]
                    : null;
                ret["badgeColor"] != null
                    ? badgeColor = ret["badgeColor"]
                    : null;
                setState(() {});
              }
            });
...
class TrackListDeckItemState {
  ...
  Map toMap() {
    ...
    this.extraData != null
        ? this.extraData!.keys.forEach((elem) {
...
class FlashingBadgeIcon extends StatefulWidget {
  ...
  FlashingBadgeIcon(
      ...
      this.child,
      this.colors,
      this.durationBetweenColorChanges = const Duration(milliseconds: 1000),
      this.IconSize,
...
class _FlashingBadgeIconState extends State<FlashingBadgeIcon> {
  ...
  Widget build(BuildContext context) {
    return Icon(
      ...
      color: flash!
          ? colors![currentIndex]
          : (nonFlashingColor ?? colors![currentIndex]),
    );
  }
