  ...
  Widget build(BuildContext context) {
    return Container(
      child: GestureDetector(
        ...
        onTapDown: (details) {
          List<PopupMenuEntry<ContextMenuOptions>> itemList =
              choices!.map((ContextMenuOptions choice) {
          ...
          if (details.globalPosition.dy >
              screenSize!.height - 210 - 10 * choices!.length) {
            YToSubstract = max(
                0.0,
                details.globalPosition.dy -
                    (screenSize!.height -
                        choices!.length * 30 -
                        (staticOffsetFromBottom ?? 160)));
          }
          showPopMenu(context, itemList,
              Buttonoffset: details.globalPosition,
              ExtraOffset: Offset(-0, -YToSubstract));
        },
      ),
    );
  }

  void showPopMenu(context, List<PopupMenuEntry<ContextMenuOptions>> items,
      {@required Offset? Buttonoffset, Offset? ExtraOffset}) async {
    ...
    RelativeRect position = RelativeRect.fromSize(
      Rect.fromPoints(
        Offset(Buttonoffset.dx + (ExtraOffset != null ? ExtraOffset.dx : 0.0),
            Buttonoffset.dy + (ExtraOffset != null ? ExtraOffset.dy : 0.0)),
        Offset(Buttonoffset.dx, Buttonoffset.dy),
      ),
      overlay!.size,
    );
    ContextMenuOptions? Choice = await showMenu<ContextMenuOptions>(
        context: context,
        position: position,
        items: items,
