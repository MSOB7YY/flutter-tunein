...
class PreferredPicks extends StatelessWidget {
  ...
  final MapEntry<double, double>? blurPower;
  ...
  PreferredPicks(
      ...
      this.key,
      this.borderRadius,
      this.allImageBlur = true})
      : super(key: key);

  @override
  Widget build(BuildContext context) {
    Color shadowColor = ((colors != null && colors!.length != 0)
            ? new Color(colors![0])
            : Color(themeService.defaultColors[0]))
        .withOpacity(.7);
    return Container(
            ...
            children: <Widget>[
              ImageFiltered(
                imageFilter: ImageFilter.blur(
                    sigmaX: blurPower != null ? blurPower!.key : 2.0 as dynamic,
                    sigmaY:
                        blurPower != null ? blurPower!.value : 3.0 as dynamic),
                child: Container(
                  clipBehavior: Clip.antiAlias,
                  decoration: BoxDecoration(
                    color: Colors.transparent,
                    borderRadius:
                        BorderRadius.all(borderRadius ?? Radius.circular(10)),
                    border: Border.all(width: .3, color: MyTheme.bgBottomBar),
                  ),
                ...
                Container(
                  ...
                  child: BackdropFilter(
                    filter: ImageFilter.blur(
                        // sigmaX: blurPower != null ? blurPower.key : 2,
                        sigmaX:
                            blurPower != null ? blurPower!.key : 2.0 as dynamic,
                        sigmaY: blurPower != null
                            ? blurPower!.value
                            : 3.0 as dynamic),
                    child: Container(
                        decoration: BoxDecoration(
