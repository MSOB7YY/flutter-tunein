  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder(
      ...
      builder: (context, AsyncSnapshot<MapEntry<PlayerState, Tune?>> snapshot) {
        ...
        Color? primaryColor = dataON && (data?.value?.colors?.isNotEmpty ?? false) ? Color(data?.value?.colors?.elementAt(0) ?? MyTheme.darkBlack.value) : MyTheme.darkBlack;
        Color secondaryColor = dataON && (data?.value?.colors?.isNotEmpty ?? false) ? Color(data?.value?.colors?.elementAt(0) ?? MyTheme.grey300.value) : MyTheme.grey300;
        return InnerDrawer(
          key: _innerDrawerKey,
          onTapClose: true,
          // default false
          swipe: true,
          // default true
          colorTransitionChild: MyTheme.darkRed.withOpacity(.5),
          ...
          backgroundDecoration: BoxDecoration(
            color: dataON && (data?.value?.colors?.isNotEmpty ?? false) ? Color(data?.value?.colors?.elementAt(0) ?? MyTheme.bgBottomBar.value) : MyTheme.bgBottomBar,
          ),
                        ...
                        slivers: <Widget>[
                          ...
                          SliverToBoxAdapter(
                            child: Column(
                              ...
                              children: <Widget>[
                                Padding(
                                  child: ((data!.value?.title!.length ?? 0) < 25)
                                      ? Text(
                                    data.value?.title ?? "Unknon Title",
                                      ...
                                      : Container(
                                          height: 15,
                                          child: Marquee(
                                            text: data.value?.title ?? "Unknon Title" ,
                                            style: TextStyle(
                                              fontSize: 13.5,
                                ...
                                Padding(
                                  ...
                                  child: Text(
                                    data.value?.artist ?? "Unknown Artist" ,
